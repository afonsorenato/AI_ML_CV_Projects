...
while True:
    ...
    frame, count = getEyeClose(frame, predictor_path, predictor, detector, count)

    # Detects if there is a phone on scene
    frame, color = getCellPhone(frame, net, classes, color)

    end = time.time()
    print("Elapsed time: ", str(round(end-start, 2)))

    frame = cv2.resize(frame, (round(frame.shape[1]/0.4), round(frame.shape[0]/0.4)))
    #frame = cv2.resize(frame, (round(frame.shape[1]/2), round(frame.shape[0]/2)))
